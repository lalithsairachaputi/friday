...
from backend.app.state.conversation import (
    ActiveTask,
    ConversationState,
    SpeechState,
    TaskRuntimeState,
...
class AgentOrchestrator:
    ...
    async def apply_interruption(
        self,
        state: ConversationState,
    ) -> int:
        self.interruptions.on_user_speech_started(
            state,
            source="vad",
        )
        ...
        self.tools.cancel_conversation(
            state.conversation_id
        )

        if state.active_task:
            state.active_task.generation_id = (
        ...
        *,
        ...
        if barge_in:
            ...
            if self.settings.interruption_delay_ms:
                await asyncio.sleep(
                    self.settings.interruption_delay_ms / 1000
                )

        decision = self.language.detect(
            raw_transcript,
        ...
        rec = self.reconciler.classify(
            ...
            language_changed=(
                decision.switched
                or decision.explicit_request
            ),
            explicit_language=decision.explicit_request,
        )

        turn_id = self.turns.start_user_turn(
            state,
            raw_transcript,
        )
        ...
        if rec.kind == ReconciliationKind.CANCELLATION:
            ...
            if state.active_task:
                state.active_task.status = (
                    TaskRuntimeState.CANCELLED
                )

            text = self._cancel_speech(
        ...
        if rec.kind in {
            ...
            state.current_constraints.update(
                rec.constraints_delta
            )

            if state.active_task:
                state.active_task.constraints.update(
                    rec.constraints_delta
                )
        ...
        if (
            rec.kind == ReconciliationKind.LANGUAGE_CHANGE
            and not state.task_is_active
        ):
            text = self._language_ack(
                decision.response_language
            )
        ...
        needs_calendar = (
            "calendar" in raw_transcript.lower()
            or "meeting" in raw_transcript.lower()
        )
        ...
        if needs_web:
            if (
                rec.kind == ReconciliationKind.NEW_REQUEST
                or state.active_task is None
            ):
            ...
            status_line = self._checking_speech(
                decision.response_language
            )
            ...
            if tool_result.ok:
                ...
                answer = (
                    "I could not finish that web search. "
                    "Ask me to try again."
                )
            ...
            if (
                tool_result.ok
                and not tool_result.stale
            ):
                events = tool_result.data.get(
                    "events",
                    [],
                )

                answer = (
                    "Your next calendar item is "
                    + (
                        events[0]["title"]
                        if events
                        else "empty"
        ...
        self.turns.append_agent(
            state,
            answer,
            interrupted=record.tts_interrupted,
        )
    ...
    def _needs_web(
        ...
        if (
            ...
            and state.active_task.kind == "web_research"
        ):
            return True
    ...
    async def _compose_answer(
        ...
        """
        Generate Friday's final response using Google Gemini.

        Gemini is accessed through the official google-genai SDK.
        The async client is used so it fits the existing async
        AgentOrchestrator architecture.
        """

        if self.settings.llm_delay_ms:
            await asyncio.sleep(
                self.settings.llm_delay_ms / 1000
            )

        # Gemini key missing -> preserve the existing fallback.
        if not self.settings.gemini_api_key:
            if (
                tool_data
                and tool_data.get("summary")
            ):
                return self._localize_fallback(
                    tool_data["summary"],
                    lang,
                )
            ...
            return self._localize_fallback(
                (
                    "I heard you. Tell me if you want "
                    "me to search the web or check "
                    "your calendar."
                ),
                lang,
            )
        ...
        try:
            ...
            # If Gemini fails but web research produced a
            # summary, return the useful tool result instead
            # of completely failing the conversation.
            if (
                tool_data
                and tool_data.get("summary")
            ):
            ...
            return self._localize_fallback(
                (
                    ...
                    "to my language model right now."
                ),
                lang,
            )
        ...
        async for piece in iter_segments(
            [text]
        ):
            spoken.append(piece)
    ...
    def _status_speech(
        ...
        return {
            ...
            "hi": (
                f"अभी कार्य की स्थिति "
                f"{status} है।"
            ),
            "ta": (
                f"தற்போதைய பணி நிலை {status}."
            ),
            "te": (
                f"ప్రస్తుత పని స్థితి {status}."
            ),
        }.get(
            lang,
            f"The current task is {status.lower()}.",
        )
    ...
    def _result(
        ...
        return {
            ...
            "code_switched": (
                decision.code_switched
            ),
            "raw_transcript": (
                state.last_user_utterance_raw
            ),
            "generation_id": (
                state.active_generation_id
            ),
            ...
            "tts": {
                "active_tts_provider": (
                    ...
                    else profile.fallback_provider
                ),
                "rime_model": profile.rime_model,
                "rime_speaker": (
                    profile.rime_speaker
                ),
                "language": (
