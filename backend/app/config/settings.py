...
class Settings(BaseSettings):
    ...
    friday_cors_origins: str = "http://localhost:8080,http://127.0.0.1:8080"
    ...
    def cors_origin_list(self) -> list[str]:
        return [item.strip() for item in self.friday_cors_origins.split(",") if item.strip()]
...
def stress_mode_active(settings: Settings | None = None) -> bool:
    ...
    return any(v > 0 for v in s.stress_flags().values())
